...
from .forms import UserAuth, UserRegisterForm, SellerRegisterForm
from ..shop.models import Buyer, Seller

def customer_profile(request):
    return render(request, 'customer/seller_profile.html')
...
def user_register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            Buyer.objects.create(user=user, phone=phone)
            login(request, user)
            return redirect('shop_home-page')
        else:
...
def seller_register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return render(request, 'customer/register_seller.html', {'form': form})
    else:
